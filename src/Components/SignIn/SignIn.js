...
class SignIn extends React.Component{
    ...
    validateElement(element){
        ...
        if(element.validation.email){
            let regex = /^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/igm;
    ...
    showButtons = ()=>{
        if(!this.state.loading){
            return(
                <div>
                    <button type="submit" onClick={(e)=> this.handleFormSubmit(e,'login')}>Login</button>
                    <button type="submit" onClick={(e)=> this.handleFormSubmit(e,'register')}>Register</button>
                </div>
            )
        }
        else{
    ...
    showRegisterError = ()=>{
        return this.state.registerError!==''? <div style={{color:"red"}} className="mt-3">{this.state.registerError}</div> : '';
    }
    ...
    render(){
        return(
            ...
            <form onSubmit={(e)=> this.handleFormSubmit(e,'direct-submit')}>
                ...
                <FormFields id="password" formData={this.state.formData.password} 
                change={(element)=> this.updateFormData(element)} 
                blur = {element => this.updateFormData(element)}
                showError = {this.handleInputError}
                />
